...
const InvoiceGenerator = () => {
  ...
  const productState = useSelector((s) => s.products || { items: [], loading: false, error: null });
  ...
  useEffect(() => setShowDropdown(Boolean(filteredPartsRedux && filteredPartsRedux.length)), [filteredPartsRedux]);
  ...
  const calcTaxFor = (mrp, qty, cgst, sgst) => (mrp * qty * (cgst + sgst)) / 100;
  ...
  const changeQty = (id, val) => {
    setItems(items.map((it) => (it.id === id ? { ...it, quantity: Number(val) || 1, amount: it.mrp * (Number(val) || 1), taxAmount: calcTaxFor(it.mrp, Number(val) || 1, it.cgst, it.sgst) } : it)));
  };
  ...
  const downloadPDF = () => {
    ...
    const invoiceHTML = `
      <div class="invoice-container">
        <div class="header">
          ...
          <div class="logo-area">
            ${logoDataUrl ? `<img src="${logoDataUrl}" alt="Logo" />` : '<div>LOGO</div>'}
          </div>
        </div>
          ...
          <tbody>
            ${items.map((item, idx) => `
              <tr>
                ...
                <td class="text-center">${(item.cgst + item.sgst).toFixed(1)}%</td>
        ...
        <div class="footer-section">
          ...
          <div class="amounts-box">
            ...
            <div class="amount-row">
              <span>SGST@${items.length && items[0].sgst ? items[0].sgst.toFixed(1) : "0"}%</span>
              <span>₹${(totalTax() / 2).toFixed(2)}</span>
            </div>
            <div class="amount-row">
              <span>CGST@${items.length && items[0].cgst ? items[0].cgst.toFixed(1) : "0"}%</span>
              <span>₹${(totalTax() / 2).toFixed(2)}</span>
            </div>
        ...
        <div class="bottom-section">
          <div class="service-info">
            <div>Next Service Date: ${nextServiceDate ? formatDate(nextServiceDate) : "--"}</div>
    ...
    printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Invoice-${invoiceNumber}</title>${printCss}</head><body>${invoiceHTML}<script>window.onload = function(){ window.print(); setTimeout(()=>window.close(),200); };</script></body></html>`);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button onClick={resetForm} className="flex items-center gap-2 border border-gray-300 text-gray-700 hover:bg-gray-100 px-4 py-2 rounded-md">
              <X size={16} /> Reset
            </button>
            <button onClick={downloadPDF} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
        ...
        <div className="bg-white p-6 rounded-lg border shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 pb-6 border-b">
            <div>
              ...
              <input type="text" value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} className="w-full border border-gray-300 p-2 rounded" />
            </div>
            <div>
          ...
          <div className="mb-6 pb-6 border-b">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <input placeholder="Contact Number" value={customerInfo.phone} onChange={(e) => setCustomerInfo({ ...customerInfo, phone: e.target.value })} className="w-full border border-gray-300 p-2 rounded" />
            </div>
          </div>
          ...
          <div className="mb-6 pb-6 border-b">
            ...
            <div className="flex gap-2 relative">
              <input
                ...
                onKeyPress={(e) => e.key === "Enter" && filteredPartsRedux.length > 0 && addPart(filteredPartsRedux[0])}
                placeholder="Search by part no or name"
                className="flex-1 border border-gray-300 p-2 rounded"
              />
              <button onClick={() => addPart()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md font-medium transition flex items-center">
              ...
              {showDropdown && (
                <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 mt-1 rounded shadow-lg z-50 max-h-60 overflow-y-auto">
                  ...
                  {!loading && filteredPartsRedux.map((part) => (
                    <div key={part.partNo} onClick={() => addPart(part)} className="px-4 py-3 hover:bg-blue-50 cursor-pointer border-b last:border-b-0">
          ...
          {items.length > 0 ? (
                ...
                <tbody>
                  {items.map((item, i) => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-2 text-center">{(item.cgst + item.sgst).toFixed(1)}%</td>
                      <td className="p-2 text-right font-medium">₹{(item.amount + item.taxAmount).toFixed(2)}</td>
                      <td className="p-2 text-center">
            ...
            <div className="text-center py-6 text-gray-500 bg-gray-50 rounded">No items added yet.</div>
          )}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-6 border-t">
            <div>
              <label className="text-sm font-medium block mb-2">Next Service Date</label>
              <input type="date" value={nextServiceDate} onChange={(e) => setNextServiceDate(e.target.value)} className="w-full border border-gray-300 p-2 rounded" />
            </div>
            <div>
              <label className="text-sm font-medium block mb-2">Next Service Kms</label>
              <input type="text" value={nextServiceKms} onChange={(e) => setNextServiceKms(e.target.value)} placeholder="e.g., 10000" className="w-full border border-gray-300 p-2 rounded" />
            </div>
          </div>
        </div>
      </div>
