...
import { useInfiniteProducts } from "@/features/products/useProduct";
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <Card className="p-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <Link to="/dashboard/productitems/itemdetails">
            <div className="h-10 w-10 flex items-center justify-center border rounded-md hover:bg-amber-50 cursor-pointer transition-colors">
              <PackageOpen className="h-5 w-5" />
            </div>
          </Link>
          <Link className="h-10 w-10 flex items-center justify-center border rounded-md hover:bg-amber-50 cursor-pointer transition-colors"
          to={'/dashboard/productitems/add'}
          >
      ...
      {debouncedSearch && (
        <div className="flex items-center justify-between mb-3">
          ...
          {products.length < (data?.pages[0]?.totalItems || 0) && (
            <p className="text-sm text-gray-500">
              Showing {products.length} of{" "}
              {data?.pages[0]?.totalItems.toLocaleString()}
            </p>
          )}
        </div>
      )}
